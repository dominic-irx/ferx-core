...
**Its objective is a lower bound, not a likelihood.** `ofv` is `NaN` after a VI fit by default rather than being filled with a number that is not comparable to a FOCE or SAEM OFV. Set `vi_final_ofv = laplace`, or chain `methods = vi, imp` with `imp_eval_only = true`, to evaluate a genuine marginal likelihood at the VI estimate. IOV and non-Gaussian endpoints are not supported. See the [VI page](../estimation/vi.qmd).
...
| `vi_final_ofv` | `none` | `none` leaves `ofv` as `NaN` — the ELBO is a lower bound, not a `−2 log L`, and is not comparable with a FOCE/SAEM OFV. `laplace` reconverges EBEs at the VI estimate and reports the FOCE objective, which is. For an importance-sampling `−2 log L`, chain `methods = vi, imp` with `imp_eval_only = true`. |
