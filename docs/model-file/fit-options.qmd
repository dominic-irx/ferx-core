...
| `vi_sigma_update` | `closed_form` | `closed_form` replaces the residual error σ with its exact ELBO maximizer each iteration, the same treatment `vi_omega_update` gives Ω; `adam` steps it by gradient descent. Adam-stepping σ is sensitive to `vi_lr` and can settle well above the maximum-likelihood value, so `closed_form` is strongly preferred — see [Variational inference](../estimation/vi.qmd). Error structures with no scalar stationary point (combined error, several σ, per-endpoint or covariate-selected error, correlated residuals, M3 BLOQ, IIV-on-RUV, FREM, a FIXed σ) fall back to `adam` and warn. |
